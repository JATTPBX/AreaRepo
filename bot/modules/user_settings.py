...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    if not data:
        ...
        dumpch, buttonkey = (
            (f"<code>{user_dict.get('dump_ch')}</code>", " ✅️ Dump CH")
        ...
        buttons.button_data(
            " ✅️ Caption" if user_dict.get("captions") else "Caption",
        ...
        if ext_filters := user_dict.get("excluded_extensions"):
            ...
            ex_ex = ""
        buttons.button_data(
            " ✅️ Extensions Filters" if ex_ex else "Extensions Filters",
        ...
        buttons.button_data(
            " ✅️ Custom Caption" if caption else "Custom Caption",
            f"userset {user_id} setdata setcap",
        )
        buttons.button_data("Back", f"userset {user_id} back")
        if caption:
            buttons.button_data(
                " ✅️ FCaption" if fnamecap else "FCaption",
        ...
        rcpathmsg, buttonkey = (
            (f"<code>{rc_path}</code>", " ✅️ RClone Path")
...
async def edit_user_settings(client: Client, query: CallbackQuery):
    ...
    if (
        ...
        await query.answer("Upss, Premium User Required!", False)
        is_modified = False
