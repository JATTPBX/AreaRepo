...
class MultiSerach:
    ...
    async def list_buttons(self):
        ...
        if self.changeQuery:
            ...
            buttons.button_data('Change Query', 'list change', 'header')
            buttons.button_data(f'{"✅️ " if user_config else ""}User Config', 'list uc _', 'header')
            ...
            if self.type:
                ...
                user_config = self.config_path.startswith(('rclone/', 'tokens/'))
                buttons.button_data('Change Query', 'list change', 'header')
                buttons.button_data(f'{"✅️ " if self.isRecursive else ""}Recursive', 'list rec', 'header')
                buttons.button_data(f'{"✅️ " if user_config else ""}User Token', 'list ut _', 'header')
