    ...
    async def list_buttons(self, mode: str = ""):
        ...
        if not mode:
            ...
            for key, value in vid_modes.items():
                buttons.button_data(
                    f"{'✅️ ' if self.mode == key else ''}{value}", f"vidtool {key}"
                )
            buttons.button_data(
                f"{'✅️ ' if self.newname else ''}Rename", "vidtool rename", "header"
            )
            ...
            if self.mode in ("vid_sub", "watermark") and await CustomFilters.sudo(
                ...
                hardsub = self.extra_data.get("hardsub")
                buttons.button_data(
                    f"{'✅️ ' if hardsub else ''}Hardsub", "vidtool hardsub", "header"
                )
                if hardsub:
                    if self.mode == "watermark":
                        buttons.button_data(
                            f"{'✅️ ' if await aiopath.exists(self.extra_data.get('subfile', '')) else ''}Sub File",
                            "vidtool subfile",
                            "header",
                        )
            ...
            match mode:
                ...
                case "quality":
                    ...
                    [
                        buttons.button_data(
                            f"{'✅️ ' if self.extra_data.get('quality') == key else ''}{key}",
                ...
                case "fontstyle":
                    ...
                    buttons.button_data(
                        f"{'✅️ ' if self.extra_data.get('boldstyle') else ''}Bold Style",
                ...
                case "fontname":
                    ...
                    [
                        buttons.button_data(
                            f"{'✅️ ' if btn == self.extra_data.get('fontname') else ''}{btn.replace('_', ' ')}",
                ...
                case "fontsize":
                    ...
                    [
                        buttons.button_data(
                            f"{'✅️ ' if str(btn) == self.extra_data.get('fontsize') else ''}{btn}",
                ...
                case "fontcolour":
                    ...
                    [
                        buttons.button_data(
                            f"{'✅️ ' if hexcolour == self.extra_data.get('fontcolour') else ''}{btn}",
